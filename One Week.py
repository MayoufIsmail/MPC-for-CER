...
# Main loop for one week (assuming each day has 96 time steps (15-minute intervals) and there are 7 days in a week)
for t in range(7 * 96):
...
plt.plot(pv_data['data.PV'][:7*96], label='PV Output')  # Plotting for one week
plt.plot(load_data['data.Load'][:7*96], label='Load Consumption')  # Plotting for one week
plt.plot(net_gen[:7*96], label='Net Generation')  # Plotting for one week

# Plot battery charge/discharge
plt.plot(pv_charge[:7*96], label='PV Charge')  # Plotting for one week
plt.plot(grid_charge[:7*96], label='Grid Charge')  # Plotting for one week
plt.plot(discharge[:7*96], label='Discharge')  # Plotting for one week

# Plot grid import/export
plt.plot(imported[:7*96], label='Grid Import')  # Plotting for one week
plt.plot(export[:7*96], label='PV Export')  # Plotting for one week

plt.xlabel('Time Step')
plt.ylabel('Power (W)')
plt.title('BSS Operation (One Week)')
plt.legend()
plt.grid(True)
plt.show()
